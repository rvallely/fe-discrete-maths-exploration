...
function BaseConverter() {
  const [inputValue, setValue] = useState('472');
  const [fromBase, setFromBase] = useState('8');
  ...
  const handleSubmit = (e) => {
    ...
    setError('');
    setOutput({});
    console.log('getting base conversion: ', JSON.stringify({inputValue, fromBase, toBase}))
  ...
  return (
   ...
   <label htmlFor="start-value" className="form-label">Start value:</label>
                ...
                <div className="mb-4 input-group">
                  <span className="input-group-text">🚖</span>
                  <input disabled type="text" className="form-control statement-font-size dark-font" id="end-value" value={output.convertedValue} />
    ...
    <div className="mt-4 mb-0 text-center">
      <button type="button" onClick={resetForm} className="btn btn-secondary text-center statement-font-size">Reset</button>
    </div>
    <div>
    </div>
      {Object.keys(output).length === 0? '': 
      <div>
      <ExponentMethod inputValue={inputValue} fromBase={fromBase} calculations={output.calculations.decimalCalculations} result={output.calculations.decimalResult}/>
      <QuotientMethod inputValue={inputValue} fromBase={fromBase} toBase={toBase} calculations={output.calculations.finalCalculations} decimalResult={output.calculations.decimalResult} finalResult={output.calculations.finalResult}/>
